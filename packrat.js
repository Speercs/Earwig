...
/**
 * Convert a standard Game.time in screeps to a compressed UTF-16 encoded string of variable length.
 */
function packTime(time) {
	let timeString = time.toString()
	timeString = timeString.padStart(Math.ceil(timeString.length / 4) * 4, '0');
	let str = '';
	for (let i = 0; i < Math.ceil(timeString.length / 4); ++i) {
		str += String.fromCharCode(parseInt(timeString.substr(i * 4, 4), 16))
	}
	return str;
}

/**
 * Convert a variable length UTF-encoded time back into the original Game.time number.
 */
function unpackTime(packedTime) {
	let time = '';
	let current;
	for (let i = 0; i < packedTime.length; ++i) {
		current = packedTime.charCodeAt(i);
		time += (current >>> 8).toString(16).padStart(2, '0'); // String.padStart() requires es2017+ target
		time += (current & 0xFF).toString(16).padStart(2, '0');
	}
	return Number(time);
}
...
function packRoomName(roomName) {
	if (typeof PERMACACHE._packedRoomNames[roomName] === "undefined") {
...
function unpackRoomName(char) {
	if (typeof PERMACACHE._unpackedRoomNames[char] === "undefined") {
...
global.packTime = packTime;
global.unpackTime = unpackTime;
