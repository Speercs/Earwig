...
function setupMemory(profileType, duration, filter) {
  if (!enabled) {
    console.log('Profiler not enabled. Use profiler.enable()')
    return;
  }
  resetMemory();
...
function wrapFunction(name, originalFunction) {
  ...
  if (originalFunction.profilerWrapped) { originalFunction.apply(this, arguments); }
  function wrappedFunction() {
    if (Profiler.isProfiling()) {
      ...
      if (depth > 0 || !getFilter()) {
        ...
        // KMP: if we are exceeding 1 cpu then spit out the details in real time.
        //if (((end - start) > 2)) console.log((end - start).toFixed(3), this, name);
      }
      if (nameMatchesFilter) {
        depth--;
      }
...
const Profiler = {
  ...
  output(passedOutputLengthLimit) {
    ...
    const header = `${now}\t\ttime\t\tavg\t\tmax\t\tfunction`;
    const footer = [
  ...
  record(functionName, time) {
    ...
    if (Memory.profiler.map[functionName].max < time) Memory.profiler.map[functionName].max = time;
  },
